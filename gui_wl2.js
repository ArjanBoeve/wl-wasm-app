...
function findWasmBinary() {
  return locateFile("gui_wl2.wasm");
}
...
function ___syscall_poll(fds, nfds, timeout) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 0, 1, fds, nfds, timeout);
  fds >>>= 0;
  try {
    var nonzero = 0;
...
function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(25, 0, 1, dirfd, path, buf, bufsize);
  path >>>= 0;
  buf >>>= 0;
  bufsize >>>= 0;
  try {
...
function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(26, 0, 1, fd, buf, len, flags, addr, addrlen);
  buf >>>= 0;
  len >>>= 0;
  addr >>>= 0;
  addrlen >>>= 0;
...
function ___syscall_recvmsg(fd, message, flags, d1, d2, d3) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(27, 0, 1, fd, message, flags, d1, d2, d3);
  message >>>= 0;
  try {
    var sock = getSocketFromFD(fd);
...
function ___syscall_symlinkat(target, dirfd, linkpath) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 0, 1, target, dirfd, linkpath);
  target >>>= 0;
  linkpath >>>= 0;
  try {
...
function ___syscall_truncate64(path, length) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 0, 1, path, length);
  path >>>= 0;
  length = bigintToI53Checked(length);
  try {
    if (isNaN(length)) return 61;
...
function ___syscall_utimensat(dirfd, path, times, flags) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(37, 0, 1, dirfd, path, times, flags);
  path >>>= 0;
  times >>>= 0;
  try {
    path = SYSCALLS.getStr(path);
...
function __mmap_js(len, prot, flags, fd, offset, allocated, addr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(38, 0, 1, len, prot, flags, fd, offset, allocated, addr);
  len >>>= 0;
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(39, 0, 1, addr, len, prot, flags, fd, offset);
  addr >>>= 0;
  len >>>= 0;
  offset = bigintToI53Checked(offset);
  try {
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall_accept4, ___syscall_bind, ___syscall_chmod, ___syscall_connect, ___syscall_faccessat, ___syscall_fchmod, ___syscall_fchown32, ___syscall_fcntl64, ___syscall_fstat64, ___syscall_ftruncate64, ___syscall_getcwd, ___syscall_getdents64, ___syscall_getpeername, ___syscall_getsockname, ___syscall_getsockopt, ___syscall_ioctl, ___syscall_listen, ___syscall_lstat64, ___syscall_mkdirat, ___syscall_newfstatat, ___syscall_openat, ___syscall_poll, ___syscall_readlinkat, ___syscall_recvfrom, ___syscall_recvmsg, ___syscall_renameat, ___syscall_rmdir, ___syscall_sendmsg, ___syscall_socket, ___syscall_stat64, ___syscall_statfs64, ___syscall_symlinkat, ___syscall_truncate64, ___syscall_unlinkat, ___syscall_utimensat, __mmap_js, __munmap_js, _emscripten_get_element_css_size, _emscripten_set_resize_callback_on_thread, _emscripten_set_wheel_callback_on_thread, _environ_get, _environ_sizes_get, _fd_close, _fd_fdstat_get, _fd_read, _fd_seek, _fd_sync, _fd_write, _getaddrinfo ];
...
var ASM_CONSTS = {
  19882528: () => {
    ...
    FS.syncfs(true, function(err) {
      if (err) console.error("IDBFS sync error:", err); else console.log("IDBFS initialized and synced!");
    });
  },
  19882726: () => {
    FS.syncfs(false, function(err) {
      if (err) console.error("Sync error:", err); else console.log("Data saved to IndexedDB!");
    });
  },
  19882853: () => {
